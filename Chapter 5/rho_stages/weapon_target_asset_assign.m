...
%     disp(solite);
    ...
    for presolind =  presolnum : -1 : 1
        ...
        if all(all(preava(:,:,presolind)==0))
            %popdec = cat(3,popdec,predec(:,:,presolind));
            %popsta = cat(1,popsta,presta(presolind,:));
            %popfit = cat(1,popfit,prefit(presolind,:));
            %poptarsurpro = cat(1,poptarsurpro,pretarsurpro(presolind,:));
            %popasssurpro = cat(1,popasssurpro,preasssurpro(presolind,:));
            for n = sollen : -1 : 1
    ...
    preava = ava(:,:,desind);
    %     prefit = fit(desind,:);
    %     pretarsurpro = tarsurpro(desind,:);
    %     preasssurpro = asssurpro(desind,:);
