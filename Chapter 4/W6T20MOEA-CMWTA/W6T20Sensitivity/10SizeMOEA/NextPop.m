function [ ElitePop , EliteNum ] = NextPop( ElitePop , EliteNum , PopSize , Neighbor , muc , mum , WeaponNum , SafePosition , SafeNum , TargetPosition , TargetNum , TargetWeight , MaxScenario , KillRange , SurviveThreshold , EnforceMatrix)
...
while IndividualCount < PopSize
    ...
    if EliteNum(1,tally) ~= 0
        ...
        if rand <= 0.8
            ...
            PreNeighborIndex = Neighbor{tally}(1,1);
            PreNeighborNum = EliteNum(1,PreNeighborIndex);
            MidNeighborIndex = Neighbor{tally}(1,2);
            MidNeighborNum = EliteNum(1,MidNeighborIndex);
            PostNeighborIndex = Neighbor{tally}(1,3);
            PostNeighborNum = EliteNum(1,PostNeighborIndex);
            NeighborSet = [];
            NeighborNum = PreNeighborNum + MidNeighborNum + PostNeighborNum;
            if PreNeighborNum ~= 0
                NeighborSet = [NeighborSet;ElitePop(1:PreNeighborNum,PreNeighborIndex)];
            end
            if MidNeighborNum ~= 0
                NeighborSet = [NeighborSet;ElitePop(1:MidNeighborNum,MidNeighborIndex)];
            end
            if PostNeighborNum ~= 0
                NeighborSet = [NeighborSet;ElitePop(1:PostNeighborNum,PostNeighborIndex)];
            end
            
            Parent2Index = randi([1,NeighborNum]);
