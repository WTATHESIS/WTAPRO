...
Fitness = reshape([Population(FrontIndex).Fitness],[2 FrontVolumn])';
...
for ObjectiveIndex = 1 : ObjectiveNum
    ...
    for i = 2 : FrontVolumn-1
        Population(AscendIndex(i,1)).CrowdingDistance = Population(AscendIndex(i,1)).CrowdingDistance + ( Population(AscendIndex(i+1,1)).Fitness(ObjectiveIndex) - Population(AscendIndex(i-1,1)).Fitness(ObjectiveIndex) ) / ( ObjectiveMax - ObjectiveMin );
